...
const fetchNearbyStations = async (lat, lon, maxDistanceMeters, signal) => {
    ...
    const res = await fetch(`${API_BASE_URL}/api/stations/nearby?${params}`, { signal });
    if (!res.ok) return [];
    return res.json();
};
...
const useSearchMarker = () => {
    ...
    const toggleDistanceLine = useCallback((stationId) => {
        setDistanceLineVisibility((prev) => ({ ...prev, [stationId]: !prev[stationId] }));
    }, []);
    ...
    const flyToGeoLocation = useCallback((lat, lon, label) => {
        ...
        const addCirclesAfterFly = async () => {
            ...
            try {
                const stations = await fetchNearbyStations(lat, lon, largestRadiusMeters, abortController.signal);
                if (abortController.signal.aborted) return;

                const distanceLines = stations.map((station) => {
                    const stationLatLon = [station.approxLocation.lat, station.approxLocation.lon];
